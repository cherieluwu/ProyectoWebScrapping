...
def verificar_datos():
    ...
    for producto in db.productos.find().sort('ultima_actualizacion', -1).limit(3):
        ...
        print(f"Tienda: {producto.get('tienda', 'No especificada')}")
        print(f"Precio: ${producto.get('precio', 'No especificado')}")
        print(f"Actualizado: {producto.get('ultima_actualizacion', 'No especificada')}")
        print("---")
